...
<article class="article"> 
  <div class="tab1" id="tab1">
    ...
    <div class="menu">
      <ul>
        <li <?php if(!($cyid > 0)): ?>class="off"<?php endif; ?>><a href="<?php echo ($config["site_url"]); ?>/mernews/<?php echo ($merid); ?>.html">全部资讯</a></li>
		<?php if(is_array($classify)): $kk = 0; $__LIST__ = $classify;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$cyv): $mod = ($kk % 2 );++$kk; if($cyid == $cyv['id']){$cyname=$cyv['cyname'];} ?>
        <li <?php if($cyid == $cyv['id']): ?>class="off"<?php endif; ?>><a href="<?php echo ($config["site_url"]); ?>/mernews/<?php echo ($merid); ?>.html?cyid=<?php echo ($cyv['id']); ?>"><?php echo ($cyv['cyname']); ?></a></li><?php endforeach; endif; else: echo "" ;endif; ?>
      </ul>
    </div>
    <div class="menudiv">
      <div id="con_one_1">
        <section class="server dynamics">
          <div class="section_title">
            <div class="section_txt"><?php echo ($cyname); ?></div>
            <div class="section_border"> </div>
            <div style="clear:both"></div>
          </div>
          <ul>
		   <?php if(!empty($introduce)): if(is_array($introduce)): $i = 0; $__LIST__ = $introduce;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$nvv): $mod = ($i % 2 );++$i;?><li>
              <div class="shop_dynamics_left"><span><img src="<?php echo ($static_path); ?>images/12322_03.png" /></span><a href="<?php echo ($config["site_url"]); ?>/newsdetail/<?php echo ($merid); ?>.html?newsid=<?php echo ($nvv['id']); ?>"><?php echo ($nvv['title']); ?></a></div>
              <div class="shop_dynamics_right"><?php echo (date("Y-m-d H:i:s",$nvv['addtime'])); ?></div>
            </li><?php endforeach; endif; else: echo "" ;endif; ?>
			<?php else: ?>
			 <li>暂无资讯</li><?php endif; ?>
          </ul>
		  <div style="clear:both"></div>
			<?php echo ($pagebar); ?>
            <div style="clear:both"></div>
