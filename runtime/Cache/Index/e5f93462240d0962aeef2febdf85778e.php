			...
			<div class="address-list cf">
				...
				<div class="biz-wrapper J-biz-wrapper biz-wrapper-nobottom inited">
					<div id="J-bizinfo-list" class="all-biz cf">
					   <?php $nowstore=$storelist['0']; ?>
						<?php if(is_array($storelist)): $i = 0; $__LIST__ = $storelist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i; if($vo['store_id']==$store_id){$nowstore=$vo;} ?>
							<div class="biz-info <?php if($i == 1): ?>biz-info--open biz-info--first<?php endif; ?> <?php if(count($storelist) == 1): ?>biz-info--only<?php endif; ?>">
								<h5 class="biz-info__title">
									<a class="poi-link" title="<?php echo ($vo["name"]); ?>" href="javascript:void(0);"><?php echo ($vo["name"]); ?></a><i class="F-glob F-glob-caret-down-thin down-arrow"></i>
								</h5>
								<div class="biz-info__content">
									<div class="biz-item field-group" title="<?php echo ($vo["area_name"]); echo ($vo["adress"]); ?>"><label class="title-label">地址：</label><?php echo ($vo["area_name"]); echo ($vo["adress"]); ?></div>
									<div class="biz-item link-item">
										<a class="view-map" href="/mermap/<?php echo ($merid); ?>.html?sid=<?php echo ($vo['store_id']); ?>">查看地图</a>
										&nbsp;&nbsp;&nbsp;
										<a class="search-path" href="javascript:void(0)" shop_name="<?php echo ($vo["adress"]); ?>">公交/驾车去这里</a>
									</div>
									<div class="biz-item"><span class="title-label">电话：</span><?php echo ($vo["phone"]); ?></div>
								</div>
...
<!--leftsead end-->
		...
		<script type="text/javascript">
			...
			//定位
			var point = new BMap.Point(<?php echo ($nowstore['long']); ?>,<?php echo ($nowstore['lat']); ?>);
			...
			var content = "<b>地址：</b><?php echo ($vo["area_name"]); echo ($nowstore['adress']); ?><br/><b>电话：</b><?php echo ($nowstore['phone']); ?><br/><b>线路：</b><a href='http://map.baidu.com/m?word=<?php echo (urlencode($nowstore['adress'])); ?>' target='_blank'>公交/驾车路线查询»</a>";
			//创建检索信息窗口对象
			var searchInfoWindow = null;
			searchInfoWindow = new BMapLib.SearchInfoWindow(map, content, {
				title  : "<?php echo ($nowstore['name']); ?>",      //标题
				width  : 290,             //宽度
				height : 60,              //高度
				panel  : "panel",         //检索结果面板
