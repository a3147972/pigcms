...
class HardwareAction extends BaseAction
{
    public function index()
    {
        ...
        foreach ($list as &$o ) {
            $o["name"] = ($tmp[$o["store_id"]]["name"] ? $tmp[$o["store_id"]]["name"] : "");
        }
    ...
    {
        $pigcms_id = ($_REQUEST["pigcms_id"] ? intval($_REQUEST["pigcms_id"]) : 0);
        $pigcms_id && ($orderprint = D("Orderprinter")->field(true)->where(array("mer_id" => $this->merchant_session["mer_id"], "pigcms_id" => $pigcms_id))->find());

        if (IS_POST) {
            $data["store_id"] = (!empty($_POST["store_id"]) ? intval($_POST["store_id"]) : 0);
            $data["username"] = (!empty($_POST["username"]) ? htmlspecialchars($_POST["username"]) : "");
            $data["mp"] = (!empty($_POST["mp"]) ? htmlspecialchars($_POST["mp"]) : "");
            $data["mcode"] = (!empty($_POST["mcode"]) ? htmlspecialchars($_POST["mcode"]) : "");
            $data["mkey"] = (!empty($_POST["mkey"]) ? htmlspecialchars($_POST["mkey"]) : "");
            $data["qrcode"] = (!empty($_POST["qrcode"]) ? $_POST["qrcode"] : "");
            $data["count"] = (!empty($_POST["count"]) ? intval($_POST["count"]) : 1);
            $data["paid"] = (!empty($_POST["paid"]) ? intval($_POST["paid"]) : 0);
            $data["mer_id"] = intval($this->merchant_session["mer_id"]);
            $data["count"] = min($data["count"], 100);

            $tobj = D("Orderprinter")->field(true)->where("`mer_id`='{$this->merchant_session["mer_id"]}' AND `store_id`='{$data["store_id"]}' AND `pigcms_id`<>'$pigcms_id'")->find();
            if (!empty($tobj)) {
