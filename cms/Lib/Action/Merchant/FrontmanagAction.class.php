...
class FrontmanagAction extends BaseAction
{
    ...
    public function withdraw()
    {
        if (IS_POST) {
            ...
            $info = D('Merchant')->where(array('account' => session('merchant.account')))->find();
            ...
            if ($result) {
                D('Merchant')->useBalance(session('merchant.mer_id'), $balance);
                session('merchant.balance', session('merchant.balance') - $balance);
                $this->success('提交申请成功');
            } else {
