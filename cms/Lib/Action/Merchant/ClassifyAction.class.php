...
class ClassifyAction extends BaseAction
{
    ...
    public function index()
    {
        ...
        $fid = $where["fid"] = ($_GET["fid"] ? intval($_GET["fid"]) : 0);
    ...
    public function insert()
    {
        ...
        $tpid = !empty($_POST["tpid"]) ? $_POST["tpid"] : "";
        $conttpid = !empty($_POST["conttpid"]) ? $_POST["conttpid"] : "";
        
        if(!is_numeric($tpid) || !is_numeric($conttpid) ){
            if($id){
                $this->error("请选择模版", U("Classify/edit", array("id" => $id,"fid" => $_POST["fid"])));
            }else{
                $this->error("请选择模版", U("Classify/add", array("fid" => $_POST["fid"])));
            }
            exit();
        }
    ...
    public function set()
    {
        ...
        if (IS_POST) {
            ...
            $data["homeurl"] = ($_POST["homeurl"] ? htmlspecialchars($_POST["homeurl"]) : "");
            $data["info"] = ($_POST["info"] ? htmlspecialchars($_POST["info"]) : "");
            $data["gzhurl"] = ($_POST["gzhurl"] ? htmlspecialchars($_POST["gzhurl"]) : "");
            $data["apiurl"] = ($_POST["apiurl"] ? htmlspecialchars($_POST["apiurl"]) : "");
            $images = $this->upload();
