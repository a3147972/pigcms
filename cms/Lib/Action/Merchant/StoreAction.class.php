...
class StoreAction extends BaseAction{
	...
	private function notice($order)
	{
		...
		if ($this->config['sms_finish_order'] == 2 || $this->config['sms_finish_order'] == 3) {
			$merchant = D('Merchant')->where(array('mer_id' => $order['mer_id']))->find();
			$sms_data['uid'] = 0;
		...
		$mer_store = D('Merchant_store')->where(array('mer_id' => $this->store['mer_id'], 'store_id' => $this->store['store_id']))->find();
		$msg = array();
		$msg['user_name'] = $order['nickname'];
		$msg['user_phone'] = $order['phone'];
		$msg['user_address'] = $order['adress'];
		$msg['user_message'] = $order['delivery_comment'];
		$msg['buy_time'] = date("Y-m-d H:i:s", $order['add_time']);
		$msg['pay_time'] = date("Y-m-d H:i:s", $order['pay_time']);
		$msg['use_time'] = date("Y-m-d H:i:s");
		$msg['goods_list'] = unserialize($order['info']);
		$msg['goods_count'] = $order['num'];
		$msg['goods_price'] = $order['total_money'];
		$msg['minus_price'] = $order['wx_cheap'];
		$msg['true_price'] = $order['total_money'] - $order['wx_cheap'];
		...
		$msg = ArrayToStr::array_to_str($msg, 1, $this->config['group_print_format']);
				...
				if ($order = D("Meal_order")->where(array('mer_id' => $this->store['mer_id'], 'order_id' => $order_id, 'store_id' => $store_id))->find()) {
					...
					if (D("Meal_order")->where(array('mer_id' => $this->store['mer_id'], 'order_id' => $order_id, 'store_id' => $store_id))->save($data)) {
						if ($status && $order['status'] == 0) {
							...
							$msg['store_phone'] = $mer_store['phone'];
							$msg['store_address'] = $mer_store['adress'];
							$msg = ArrayToStr::array_to_str($msg, 1, $this->config['print_format']);
							$op = new orderPrint($this->config['print_server_key'], $this->config['print_server_topdomain']);
							$op->printit($this->store['mer_id'], $store_id, $msg, 1);
