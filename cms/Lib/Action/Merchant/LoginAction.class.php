...
class LoginAction extends Action
{
    ...
    public function reg_check()
    {
        ...
        if (true) {
            ...
            if (!empty($recomment)) {
                $checkUser = D('User')->checkUser($recomment);
                if ($checkUser) {
                    $recomment = $checkUser;
                    $recomment_type = 1;
                } else {
                    $recomment = D('Merchant')->getInfo($recomment);
                    if ($recomment) {
                        $recomment_type = 2;
                    } else {
            ...
            $_POST['login_count'] = 0;
            $_POST['reg_from'] = 0;
            $_POST['invit_type'] = $recomment_type;
            $_POST['invitcode'] = $invitcode;
            $_POST['recomment'] = $recomment;

            if ($insert_id = $database_merchant->data($_POST)->add()) {
                M('Merchant_score')->add(array('parent_id' => $insert_id, 'type' => 1));
                D('InviteCode')->usedCode($invitcode);
                if ($config['merchant_verify']) {
