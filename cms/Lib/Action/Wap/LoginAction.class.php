    ...
    public function weixin_bind_reg()
    {
        if (IS_POST) {
            ...
            if (!empty($user_import)) {
                ...
                $data_user['now_money'] = $user_import['money'];
                $data_user['importid'] = $user_import['id'];
                $data_user['bank_name'] = I('post.bank_name');
                $data_user['bank_address'] = I('post.bank_address');
                $data_user['bank_code'] = I('post.bank_code');
                $data_user['bank_account'] = I('post.bank_account');
                $data_user['alipay_name'] = I('post.alipay_name');
                $data_user['alipay_account'] = I('post.alipay_account');
                $data_user['recomment'] = I('post.recomment', '');
                $data_user['id_number'] = I('post.id_number', '');
                $data_user['id_number_img'] = I('post.id_number_img', '');
                $mer_id = $user_import['mer_id'];
                if ($mer_id > 0) {
                    $merchant_user_relationDb = M('Merchant_user_relation');
                    $mwhere = array('openid' => $data_user['openid'], 'mer_id' => $mer_id);
