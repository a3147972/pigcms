...
// 		}
    ...
    public function companyDetail(){
		...
		$where = array('mer_id'=>$this->token);
		$companies = M('Merchant_store')->where($where)->order('mer_id ASC')->select();
...
//     		var_dump($thisCard);die;
    ...
    public function action_myCoupon(){
    	...
    	if ($data['coupon_type'] == 3) {
    		...
    		$cwhere = array('token'=>$this->token,'wecha_id'=>$this->user_session['uid'],'cardid'=>$data['cardid'],'coupon_type'=>$data['coupon_type'],'coupon_id'=>$data['coupon_id']);
    		$count 	= M('Member_card_coupon_record')->where($cwhere)->count();
    ...
    public function coupon(){
    	...
    	$is_use 	= $this->_get('is_use','intval')?$this->_get('is_use','intval'):'0';
    	$now=time();
		$where 	= array('token'=>$this->token,'wecha_id'=>$this->user_session['uid'],'cardid'=>$thisCard['id'],'coupon_type'=>$type,'is_use'=>"$is_use");
    ...
    public function action_useIntergral()
    {
    	...
    	if (IS_POST) {  		
			...
			if (md5($this->_post('password'))!=$thisStaff['password']) {
				...
				$arr['itemid']	= $itemid;
				//更新记录
				M('Member_card_use_record')->where(array('token'=>$this->token,'wecha_id'=>$this->user_session['uid'],'itemid'=>$r_record['id']))->save($arr);
