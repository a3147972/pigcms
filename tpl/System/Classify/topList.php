    ...
    text-align: center;
		...
		<div class="mainbox">
			...
			<table class="search_table" width="100%">
				<tr>
					<td>
						<form action="{pigcms{:U('Classify/topList')}" method="get">
						...
						<input type="hidden" value="topList" name="a"></input>
							...
							<option value="{pigcms{$vo['cid']}"  <if condition="$vo['cid'] eq $cid">selected="selected"</if>>{pigcms{$vo['cat_name']}</option>
							</volist>
							<else/>
							...
							<option value="{pigcms{$v2o['cid']}" <if condition="$v2o['cid'] eq $subdir2cid">selected="selected"</if>>{pigcms{$v2o['cat_name']}</option>
							</volist>
							</select>
							<else/>
						...
						<tbody id="drag_table">
							<if condition="!empty($listdatas)">
								<volist name="listdatas" id="vo">
									<tr class="drag_line" id="toplist_{pigcms{$vo.id}" action-id="{pigcms{$vo.id}">
									   <td>{pigcms{$vo.id}</td>
									    <td><div class="drag_orders" action-id="{pigcms{$vo.id}">拖动排序</div></td>
										...
										<td>{pigcms{$vo.endtoptime|date='Y-m-d H:i:s',###}</td>
										<td class="red">已审核/已置顶&nbsp;&nbsp;<input type="button" value="取消置顶" style="" class="button" onclick="CancelTop({pigcms{$vo.id})"></td>
										<td class="textcenter"><a href="javascript:void(0);" onclick="window.top.artiframe('{pigcms{:U('Classify/infodetail',array('vid'=>$vo['id']))}','查看信息详情',680,560,true,false,false,closebtn,'edit',true);">查看详细</a>&nbsp; | <a href="javascript:void(0);" onclick="window.top.artiframe('{pigcms{:U('Classify/attrSet',array('vid'=>$vo['id']))}','设置操作',550,350,true,false,false,confirmbtn,'add',true);">设置操作</a>&nbsp; | &nbsp;<a href="javascript:void(0);" onclick="toDelItem({pigcms{$vo.id});">删除</a></td>
									</tr>
								</volist>
...
var Cid="{pigcms{$subdir2cid}";
var fCid="{pigcms{$cid}";
</script>
<script type="text/javascript" src="{pigcms{$static_public}js/DragList.js"></script>
<script type="text/javascript">
...
$("#drag_table").DragList({
	...
	id:'action-id',
	linePre:'toplist_',
	JSONUrl:"{pigcms{:U('Classify/topsort',array('cid'=>$subdir2cid,'fcid'=>$cid))}",
	maskLoadIcon:''
});
...
function CancelTop(vid){
   if(confirm('您确定要取消置顶吗？')){
       $.post("{pigcms{:U('Classify/CancelTop')}",{vid:vid},function(data){
	    if(!data){
		  alert('取消成功！');
          window.location.reload();
	     }
