...
<head>
    <meta charset="utf-8"/>
	<title><if condition="$now_order['meal_type']">外卖<else />{pigcms{$config.meal_alias_name}</if>评价</title>
...
<body id="index" data-com="pagecommon">
        <header  class="navbar">
            ...
            <h1 class="nav-header"><if condition="$now_order['meal_type']">外卖<else />{pigcms{$config.meal_alias_name}</if>评价</h1>
            ...
            <div id="nav-dropdown" class="nav-dropdown">
                <ul>
                    <li><a class="react" href="{pigcms{:U('Home/index')}"><i class="text-icon">⟰</i>
                        ...
                        <space></space>我的</a>
                    </li><li><a class="react" href="{pigcms{:U('Search/index',array('type'=>'meal'))}"><i class="text-icon">⌕</i>
                        <space></space>搜索</a>
                </li></ul>
            </div>
        </header>
        <div id="tips" class="tips"></div>
        <form id="form" method="post" action="{pigcms{:U('My/meal_feedback',array('order_id'=>$now_order['order_id']))}">
			<dl class="list">
				<dd class="dd-padding">
					<h6>评分</h6>
					<div class="score">
	...
	$(function() {
        ...
        var imgUpload = new ImgUpload({
            ...
            countNum: "#uploadNum",
			url:"http://" + location.hostname + "/wap.php?g=Wap&c=My&a=ajaxImgUpload&ml=meal"
		  })
		 }
	  });
	</script>	
