...
<html>
	...
	<body id="index" data-com="pagecommon">
				...
				<dl class="list <if condition="$vo['record_id'] eq $_GET['card_id']">active</if>">
					<dd class="address-wrapper">
						<a class="react" href="{pigcms{$vo.select_url}">
							<div class="address-select"><input class="mt" type="radio" name="addr" <if condition="$vo['record_id'] eq $_GET['card_id']">checked="checked"</if>/></div>
							<div class="address-container">
								<div class="kv-line">
									<h6>名称：</h6><p>{pigcms{$vo.title}</p>
								</div>
								<div class="kv-line">
		...
		<else/>
			<div id="tips" class="tips" style="display:block;">您没有该商家的优惠券</div>
		</if>
