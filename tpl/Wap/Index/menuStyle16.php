...
.top_bar .top_menu>li.home a { height: 60px; width: 60px; margin: auto; border-radius: 60px; position: relative; top: -14px; background: url('tpl/Wap/static/images/home.png') no-repeat center center; background-size: 100% 100%; }
...
<li class="home"><a href="/index.php?g=Wap&c=Index&a=index&token={pigcms{$token}"></a></li>
    <volist name="catemenu" id="vo">
        <li class="litype"> <a <?php if($vo['vo']){echo 'onclick="javascript:displayit('.$vo['k'].')"';}else{echo 'href="'.$vo['url'].'"';}?>><img src="{pigcms{$vo.picurl}"><label>{pigcms{$vo.name}</label></a>
        <?php if ($vo['vo']){
        	?>
