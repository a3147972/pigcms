...
<meta charset="utf-8">
<link href="{pigcms{$static_path}tpl/189/css/cate.css" rel="stylesheet" type="text/css" />
...
<ul>
	<volist name="info" id="vo">
		<if condition="$i elt 3">
			<a href="<if condition="$vo['url'] eq ''">{pigcms{:U('Wap/Index/lists',array('classid'=>$vo['id'],'token'=>$vo['token']))}<else/>{pigcms{$vo.url|htmlspecialchars_decode}</if>"><img src="{pigcms{$vo.img}" /><p>{pigcms{$vo.name}</p></a> 
		</if>
	</volist>
