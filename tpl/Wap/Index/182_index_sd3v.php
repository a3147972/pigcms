...
<meta charset="utf-8">
<link href="{pigcms{$static_path}tpl/182/css/cate.css" rel="stylesheet" type="text/css" />
...
<div class="mainmenu">
<img src="{pigcms{$static_path}tpl/182/images/5_top.jpg"  height="15" class="sc" />
    
    
    <volist name="info" id="vo">
		
		<if condition="$i%3 == 1"><div><ul></if>
	
			<li><a href="<if condition="$vo['url'] eq ''">{pigcms{:U('Wap/Index/lists',array('classid'=>$vo['id'],'token'=>$vo['token']))}<else/>{pigcms{$vo.url|htmlspecialchars_decode}</if>" ><img src="{pigcms{$vo.img}" /><p>{pigcms{$vo.name}</p></a>
                       
		<if condition="$i%3 == 0 || count($info) == $i">
			</ul>
			<img src="{pigcms{$static_path}tpl/182/images/5_gc.jpg" height="19"  class="gc"/>
		</div>
		</if>
     </volist>   


        <img src="{pigcms{$static_path}tpl/182/images/5_bottom.jpg"  height="8" class="bottom" />
</div>
