...
<meta charset="utf-8">
<link href="{pigcms{$static_path}tpl/1135/css/cate.css" rel="stylesheet" type="text/css" />
...
<script src="{pigcms{$static_path}tpl/com/js/swipe2.js" type="text/javascript"></script>
        ...
        <div id="nav_uls" style=" max-width:75px">
          <ol>
				...
				<volist name="info" id="vo">
					<li><a href="javascript:swiper_2.slide({pigcms{$i-1});"  <if condition="$i == 1">class="on"</if>>{pigcms{$vo.name}</a></li>
				</volist>
        </div>
      </ul>
  ...
  <div id="list_uls" class="list_uls box_swipe">
    ...
    <ul >
		<volist name="info" id="vo">
			  <li>
				<dl>
				 <img  class="catebanner" src="{pigcms{$vo.img}"   />
					<volist name="vo['sub']" id="res">
						  <dd>
							<a href="<if condition="$res['url'] eq ''">{pigcms{:U('Wap/Index/lists',array('classid'=>$res['id'],'token'=>$vo['token']))}<else/>{pigcms{$res.url|htmlspecialchars_decode}</if>">
								...
								<div class="boxtext"><label>{pigcms{$res.name}</label>
								<p>{pigcms{$res.info}</p></div>
								<span></span>
							</a>
						  </dd>
					</volist>
