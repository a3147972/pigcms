...
<link href="{pigcms{$static_path}tpl/1104/css/cate.css" rel="stylesheet" type="text/css" />
 <style>
...
<ul id="thelist">
				<volist name="flashbg" id="so">    
					<li><p>{pigcms{$so.info}</p><img src="{pigcms{$so.img}"></li>
				</volist>
...
<ul> <div id="insert1" ></div>
	<volist name="info" id="vo">
		...
		<li><a href="<if condition="$vo['url'] eq ''">{pigcms{:U('Wap/Index/lists',array('classid'=>$vo['id'],'token'=>$vo['token']))}<else/>{pigcms{$vo.url|htmlspecialchars_decode}</if>" ><img src="{pigcms{$vo.img}" />{pigcms{$vo.name}</a></li>
