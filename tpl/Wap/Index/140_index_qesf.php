...
<meta charset="utf-8">
<link href="{pigcms{$static_path}tpl/140/css/cate.css" rel="stylesheet" type="text/css" />
	   ...
	   <volist name="info" id="vo">
			<li>
				<a href="<if condition="$vo['url'] eq ''">{pigcms{:U('Wap/Index/lists',array('classid'=>$vo['id'],'token'=>$vo['token']))}<else/>{pigcms{$vo.url|htmlspecialchars_decode}</if>" >
					<i><img src="{pigcms{$vo.img}" /></i>
					<p><b>{pigcms{$vo.name}</b><span>{pigcms{$vo.info}</span></p>
				</a>
			</li>
		</volist> 
	</ul>  
