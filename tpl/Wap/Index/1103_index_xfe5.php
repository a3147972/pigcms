...
<meta charset="utf-8">
<link href="{pigcms{$static_path}tpl/1103/css/cate.css" rel="stylesheet" type="text/css" />
...
<ul  class="mainmenu">
	...
	<volist name="info" id="vo">
		<li><a href="<if condition="$vo['url'] eq ''">{pigcms{:U('Wap/Index/lists',array('classid'=>$vo['id'],'token'=>$vo['token']))}<else/>{pigcms{$vo.url|htmlspecialchars_decode}</if>" >
			<div class="m_left"><img src="{pigcms{$vo.img}" /><p >{pigcms{$vo.name}</p></div>
			<p class="m_right"><span>{pigcms{$vo.info}</span><em></em></p>
		</a></li>
	</volist>	
