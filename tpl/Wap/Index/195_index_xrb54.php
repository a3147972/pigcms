...
<meta charset="utf-8">
<link href="{pigcms{$static_path}tpl/195/css/cate.css" rel="stylesheet" type="text/css" />
...
<ul  class="mainmenu">
    <volist name="info" id="vo">
		<li><a href="<if condition="$vo['url'] eq ''">{pigcms{:U('Wap/Index/lists',array('classid'=>$vo['id'],'token'=>$vo['token']))}<else/>{pigcms{$vo.url|htmlspecialchars_decode}</if>" ><img src="{pigcms{$vo.img}" /><p><b>{pigcms{$vo.name}</b><span>{pigcms{$vo.info}</span><i></i></p></a></li>
	</volist>	
</ul>
