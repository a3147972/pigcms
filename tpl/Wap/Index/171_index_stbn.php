...
<meta charset="utf-8">
<link href="{pigcms{$static_path}tpl/171/css/cate.css" rel="stylesheet" type="text/css" />
			...
			<if condition="$i%3 == 1"><ul></if>
				<li>
					<a href="<if condition="$vo['url'] eq ''">{pigcms{:U('Wap/Index/lists',array('classid'=>$vo['id'],'token'=>$vo['token']))}<else/>{pigcms{$vo.url|htmlspecialchars_decode}</if>">
						<p class="menutitle">{pigcms{$vo.name}</p>
						<p class="menuimg"><img src="{pigcms{$vo.img}" /></p>
					</a>
				</li>	
			<if condition="$i%3 == 0"></ul></if>
		</volist>

    
		<volist name="info" id="vo" offset="3" length="2">
			<if condition="$i%2 == 1"><ul></if>
				<li>
					<a href="<if condition="$vo['url'] eq ''">{pigcms{:U('Wap/Index/lists',array('classid'=>$vo['id'],'token'=>$vo['token']))}<else/>{pigcms{$vo.url|htmlspecialchars_decode}</if>">
						<p class="menutitle">{pigcms{$vo.name}</p>
						<p class="menuimg"><img src="{pigcms{$vo.img}" /></p>
