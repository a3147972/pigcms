<html>    <head>
       <if condition="$zd['status'] eq 1">
        ...
        <link href="{pigcms{$static_path}css/allcss/cate15_{pigcms{$tpl.color_id}.css" rel="stylesheet" type="text/css" />
		
       <!--  <link rel="stylesheet" type="text/css" href="{pigcms{$static_path}css/114/iscroll.css" media="all"> -->
        <script type="text/javascript" src="{pigcms{$static_path}css/114/iscroll.js"></script>
...
<include file="Index:music"/>
    ...
    <div id="scroller" style="width: {pigcms{$num*260}px; -webkit-transition: 0ms; transition: 0ms; -webkit-transform: translate3d(-496.5535668722837px, 0px, 0px) scale(1);">
    <ul id="thelist">
    ...
    <li>
        ...
        <!-- <div class="mainbg">
                <img src="<if condition="$homeInfo.homeurl neq false">{pigcms{$homeInfo.homeurl}<else/>{pigcms{$static_path}images/themes/bg1.jpg</if> "/>
        </div> -->
        <!-- <div class="mainbg">
            <volist name='flashbg' id='so'>
...
<ul><div id="insert1" ></div>
 ...
 <volist name="info" id="vo">
                    <li>
                        <div class="menubtn"><a href="<if condition="$vo['url'] eq ''">{pigcms{:U('Wap/Index/lists',array('classid'=>$vo['id'],'token'=>$vo['token']))}<else/>{pigcms{$vo.url|htmlspecialchars_decode}</if>">{pigcms{$vo.name}</a>
                        </div>
