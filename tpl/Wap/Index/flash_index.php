...
<meta charset="utf-8">
<link href="{pigcms{$static_path}css/flash/css/news2.css" rel="stylesheet" type="text/css" />
...
<li><a href="<if condition="$vo['url'] eq ''">{pigcms{:U('Wap/Index/lists',array('classid'=>$vo['id'],'token'=>$vo['token']))}<else/>{pigcms{$vo.url|htmlspecialchars_decode}</if>">{pigcms{$vo.name}</a></li>
