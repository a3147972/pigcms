...
<script src="{pigcms{$static_path}js/wScratchPad.js" type="text/javascript"></script>
</if>
</head>
...
<if condition="$Guajiang['usenums'] eq 3">
	<div class="main">
		<div class="banner"><img src="{pigcms{$static_path}images/activity-scratch-card-end2.jpg"></div>
...
<else/>
	<div class="main">
<if condition="$Guajiang['usenums'] neq 2">	
<div class="cover">
	<img src="{pigcms{$static_path}images/activity-scratch-card-bannerbg.png">
...
<div class="content">
	<!--zjl start-->
	<if condition="$wecha_id neq ''">
	<div id="zjl" <?php if($Guajiang['usenums'] != 2){?> style="display:none" <?php }?> class="boxcontent boxwhite">
		<div class="box">
			<div class="title-red"><span><if condition="$Guajiang['usenums'] neq 2">恭喜你中奖了<else/>您已经中过奖了</if></span></div>
			<div class="Detail">
				<p>您中了：<span class="red">{pigcms{$Guajiang.winprize}</span></p>
				<p class="red"> {pigcms{$Guajiang.sttxt} </p>
				<if condition="$record.phone eq ''">
            <p>
            <input name=""  class="px" id="tel" value="{pigcms{$fans.tel}" type="text" placeholder="请输入您的{pigcms{$Guajiang.renametel}">
            <input name=""  class="px" id="wechaname" value="{pigcms{$fans.wxid}" type="text" placeholder="请输入您的微信号">
            <input name=""  id="wechaid" value="{pigcms{$record.wecha_id}" type="hidden">
            <input name=""  id="lid" value="{pigcms{$record.lid}" type="hidden">
...
<div class="boxcontent boxwhite">
						<div class="box">
							...
							<div class="Detail">
				<p style="color:#f00;line-height:160%">您可能是从朋友圈等分享过的页面打开的链接，无法直接参与此活动，如需参与此活动请按照以下步骤操作：<br>1、关注微信名称“{pigcms{$config.wechat_name}”或者微信号“{pigcms{$config.wechat_id}”<br>2、输入关键词：“{pigcms{$Guajiang.keyword}”</p>            
               </div>
</div>
</div>
			</if>
	...
	<div class="boxcontent boxwhite">
		<div class="box">
			...
			<p>每人最多有{pigcms{$Guajiang.canrqnums}次机会<if condition="$Guajiang.daynums neq 0">，每天只能抽{pigcms{$Guajiang.daynums}次</if></p>
			<p>这是您第 <span class="red" id="usenums">{pigcms{$Guajiang.usecout}</span> 次刮奖</p>
			 {pigcms{:$prizeStr}
			</div>
		</div>
	</div>
...
<div class="footFix" id="footReturn"><a href="javascript:void(0)" onClick="location.href='{pigcms{:U('Wap/Index/index',array('token'=>$_GET['token']))}';"><span>返回商家首页</span></a></div>
		...
		var useragent = window.navigator.userAgent.toLowerCase();
            $("#scratchpad").wScratchPad({
                ...
                scratchMove: function () {
                    num++;
                    if (num == 2) {
                        document.getElementById('prize').innerHTML ="{pigcms{$Guajiang.winprize}";
                    }
                    if (zjl>0 && num > 5 && goon) {
                        //$("#zjl").fadeIn();
                        goon = false; 
...
$("#save-btn").bind("click",
	function() {
		...
		if (tel == '') {
	        alert("请认真输入{pigcms{$Guajiang.renametel}");
	        return
    	}
		if(wxname == ''){
			alert("请认真输入微信号");
		...
		var submitData = {
			...
			wxname	: wxname,
			wechaid : wechaid,
			lid 	: {pigcms{$Guajiang.id},
			rid 	: {pigcms{$record.id},
			action 	: "add"
		};
		$.post('wap.php?g=Wap&c=Guajiang&a=add', submitData,
			function(data) {
...
function () {
	var submitData = {
		id: {pigcms{$record.lid},
		rid: {pigcms{$record.id},
		parssword: $("#parssword").val()
	};
	$.post('wap.php?g=Wap&c=Guajiang&a=exchange', submitData,
	function (data) {
		if (data.success == true) {
			alert(data.msg);
...
window.shareData = {  
            "moduleName":"Guajiang",
            "moduleID":"{pigcms{$Guajiang.id}",
            "imgUrl": "{pigcms{$Guajiang.starpicurl}", 
            "sendFriendLink": "{pigcms{$config.site_url}{pigcms{:U('Guajiang/index',array('token'=>$token,'id'=>$Guajiang['id']))}",
            "tTitle": "{pigcms{$Guajiang.title}",
            "tContent": ""
};
</script>
{pigcms{$shareScript}
