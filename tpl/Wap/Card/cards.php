...
<li><a href="/wap.php?g=Wap&c=Card&a=integral&token={pigcms{$token}&cardid={pigcms{$card.id}"><span>我的礼品卷<?php if ($integralCount>0){echo '<em class="ok">'.$integralCount.'</em>';}else{echo '<em class="error">0</em>';}?></span></a></li>
...
<li><a href="/wap.php?g=Wap&c=Card&a=signscore&token={pigcms{$token}&cardid={pigcms{$card.id}"><span>签到赚积分<?php if ($todaySigned){echo '<em class="ok">今日已签到</em>';}else{echo '<em class="error">今日未签到</em>';}?></span></a></li>
